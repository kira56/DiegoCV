...
export let updateDepartamento = (req: Request, res: Response) => {
    Departamento.update(
        {
            dept_nom: req.body.Departamento.dept_nom
        },
        {
            where:{
                dept_id:req.body.Departamento.dept_id
            }
        }).then(()=>{
            
            Departamento.findByPk(req.body.Departamento.dept_id).then((objDepartamento:any)=>{
                res.status(200).json({
                    message:'ok',
